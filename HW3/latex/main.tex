...
将 Fisher 方程 $\partial_t u = \partial_{xx} u + u(1-u)$ 分裂为非线性部分 $\mathcal{R}$ 和线性部分 $\mathcal{D}$:
\begin{align*}
    \mathcal{R}: \quad & \partial_t u = u(1-u), \\
    \mathcal{D}: \quad & \partial_t u = \partial_{xx} u.
\end{align*}
时间步长为 $\tau$,从 $t_n$ 到 $t_{n+1}$ 的二阶 Strang 分裂格式为:
\begin{equation}
    u^{n+1} = \mathcal{S}_{\mathcal{R}}(\tau/2) \circ \mathcal{S}_{\mathcal{D}}(\tau) \circ \mathcal{S}_{\mathcal{R}}(\tau/2) u^n.
\end{equation}
其中 $\mathcal{S}_{\mathcal{R}}(t)$ 为反应步的精确解算子:
\begin{equation}
    u(t) = \frac{u_0}{u_0 + (1-u_0)e^{-t}}.
\end{equation}

\textbf{2. 空间离散}

对于扩散步 $\partial_t u = \partial_{xx} u, \ x \in [-L, L]$,作变量代换 $x = L\xi, \ \xi \in [-1, 1]$.
令 $u(\xi, t) = v(\xi, t) + u_B(\xi)$,其中 $u_B(\xi) = \frac{1-\xi}{2}$ 用于处理非齐次边界条件,使得 $v(\pm 1, t) = 0$.
选取基函数 $\phi_k(\xi) = L_k(\xi) - L_{k+2}(\xi)$,令 $v_N(\xi, t) = \sum_{k=0}^{N-2} \hat{v}_k(t) \phi_k(\xi)$.
代入方程并利用 Galerkin 投影,得到线性系统:
\begin{equation}
    M \frac{d\hat{\mathbf{v}}}{dt} = -\frac{1}{L^2} S \hat{\mathbf{v}}.
\end{equation}
其中 $M_{kj} = (\phi_j, \phi_k)$ ,$S_{kj} = (\phi'_j, \phi'_k)$.
利用 Crank-Nicolson 格式进行时间离散:
\begin{equation}
    \left( M + \frac{\tau}{2L^2}S \right) \hat{\mathbf{v}}^{} = \left( M - \frac{\tau}{2L^2}S \right) \hat{\mathbf{v}}^*,
\end{equation}
其中 $\hat{\mathbf{v}}^*$ 为反应半步后的解在谱空间中的投影系数.
\newpage
\textbf{3. 数值结果}
