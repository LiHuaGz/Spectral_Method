...
\begin{equation}
  \left\{
    \begin{matrix}
    \begin{aligned}
    &\int_{0}^{2\pi} q(x)u_N(x)e^{-imx} dx = 2\pi \tilde{u}_m\hat{q}_{m},\\
...
故原问题可化为求解线性方程组:
\begin{equation}
  \sum_{k=-N/2}^{N/2-1}mk\hat{p}_{m-k}\tilde{u}_k+\hat{q}_{m}\tilde{u}_m=\hat{f}_m, \quad m=-N/2,\cdots,N/2-1.
  \label{eq:T2_3}
\end{equation}

(2) Fourier-Galerkin伪谱方法
...
若直接使用式(\ref{eq:T2_3})左端的卷积形式计算$\mathcal{L}_1(\tilde{u})$, 则每次计算都需要$O(N^2)$的时间复杂度. 为了提高计算效率, 使用伪谱方法计算$\mathcal{L}_1(\tilde{u})$. 计算流程如下:
(1) 设当前迭代点的$\tilde{u}_N$已知(注意本题使用了共轭梯度法求解方程(\ref{eq:T2_4})), 通过IFFT计算$u_N'(x_j)$, 其中$x_j=\frac{2\pi j}{N}, j=0,1,\cdots,N-1$为配点;
...
使用共轭梯度法求解(容忍精度$10^{-10}$), $N$与$L^2$误差的关系如图\ref{fig:Q2_spectral1}所示. 从图中可以观测到：$L^2$误差从 $N=100$ 时的 $10^{-1}$ 量级迅速降至 $N \approx 250$ 时的 $10^{-7}$ 量级以下, 这表明该伪谱方法对于求解光滑周期解问题具有谱精度; 当 $N$ 增加到约500之后, $L^2$ 误差不再下降, 这可能是由于$\mathcal{L}_1+\mathcal{L}_2$不是自伴算子, 从而共轭梯度法不能保证收敛; 后面的误差上升可能是由累积误差所致(也可能是我的实现方式不对?).

\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth]{./figures/Q2b_F_spectral_Error_Analysis.png}
  \caption{伪谱方法$N$与$L^2$误差的关系}
  \label{fig:Q2_spectral1}
\end{figure}

\subsection*{2.(c)}
达到10位精度时共轭梯度法所需循环次数与$N$的关系如图\ref{fig:Q2_spectral2}所示. 由图可知, CG迭代次数与 $N$ 近似成正比(例如 $N=1000$ 时迭代约1000次,$N=4000$ 时迭代约4000次), 这说明共轭梯度法并没有因为提前达到10位精度而提前终止, 应证了上述``可能由于$\mathcal{L}_1+\mathcal{L}_2$不是自伴算子, 导致使用共轭梯度法不能保证收敛".

\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth]{./figures/Q2c_F_spectral_CG_Iterations.png}
  \caption{共轭梯度法所需循环次数与$N$的关系}
  \label{fig:Q2_spectral2}
\end{figure}
...
使用高斯消元法求解时, $N$与$L^2$误差的关系如图\ref{fig:Q2d_F_collocation_Error_Analysis_GE}所示. 可见$L^2$ 误差随 $N$ 增加而呈指数级快速下降, 在 $N \approx 500$ 之后,误差达到了 $10^{-11}$ 量级, 这代表了由双精度浮点数运算所决定的机器精度极限. 该结果验证了 Fourier 配点法的谱精度.
\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth]{./figures/Q2d_F_collocation_Error_Analysis_GE.png}
  \caption{配点法$N$与$L^2$误差的关系}
